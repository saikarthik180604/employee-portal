...
import LDManagement from "./L&DManagement";
import SuccessionPlanning from "./SuccessionPlanning";
import GrievanceResolution from "./GrievanceResolution";
import ExitProcessing from "./ExitProcessing";
...
export default function AdminDashboard({
  ...
  const fetchPerformance = useCallback(async () => {
    try {
      // We fetch EVERYTHING (*) from the table without relying on a DB Join
      ...
      // MANUAL MAPPING: We match the employee_id in the review 
      // to the 'employees' list already in your React state.
      const mappedPerformances = (data || []).map(perf => {
        const emp = employees.find(e => 
          e.id === perf.employee_id || 
          e.employeeId === perf.employee_id
        );
        
        return {
          ...perf,
          // If match found, use name; otherwise, show the ID for debugging
          employeeName: emp ? emp.fullName : `ID: ${perf.employee_id}` 
        };
      });
  ...
  }, [employees]); // Re-runs if the employee list changes
  ...
  // Ensures the fetch happens as soon as the component/employees load
  useEffect(() => {
    ...
    <div className="admin-container">
      <div className="sidebar">
        ...
        <ul className="nav-links">
          <li className="nav-section-title" role="separator">Overview</li>
          <li className={activeTab === "dashboard" ? "active" : ""} onClick={() => setActiveTab("dashboard")}>Dashboard</li>

          <li className="nav-section-title" role="separator">Talent</li>
          <li className={activeTab === "workforce" ? "active" : ""} onClick={() => setActiveTab("workforce")}>Workforce Planning</li>
          <li className={activeTab === "candidateEvaluation" ? "active" : ""} onClick={() => setActiveTab("candidateEvaluation")}>Candidate Evaluation</li>
          <li className={activeTab === "screening" ? "active" : ""} onClick={() => setActiveTab("screening")}>Screening Interviews</li>
          <li className={activeTab === "onboardingSetup" ? "active" : ""} onClick={() => setActiveTab("onboardingSetup")}>Onboarding</li>
          <li className={activeTab === "learning" ? "active" : ""} onClick={() => setActiveTab("learning")}>L&D Management</li>

          <li className="nav-section-title" role="separator">Operations</li>
          ...
          <li className="nav-section-title" role="separator">Employee Lifecycle</li>
          <li className={activeTab === "succession" ? "active" : ""} onClick={() => setActiveTab("succession")}>Succession Planning</li>
          <li className={activeTab === "grievanceResolution" ? "active" : ""} onClick={() => setActiveTab("grievanceResolution")}>Grievance Resolution</li>
          <li className={activeTab === "exitProcessing" ? "active" : ""} onClick={() => setActiveTab("exitProcessing")}>Exit Processing</li>
      ...
      <div className="main-area">
        <div className="topbar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div className="stats-summary">
            ...
            <span>{leaves.filter(l => l.status === "Pending").length} Pending Leaves</span>
          </div>
        </div>
              ...
              {activeTab === "dashboard" && (
                <div className="dashboard-wrapper">
                  ...
                  <div className="dashboard-stats">
                    <div className="stat-box blue"><div className="stat-icon" aria-hidden="true"></div><span>Total Employee</span><h3>{employees.length}</h3></div>
                    <div className="stat-box purple"><div className="stat-icon" aria-hidden="true"></div><span>Active Tasks</span><h3>{tasks.length}</h3></div>
                    <div className="stat-box green"><div className="stat-icon" aria-hidden="true"></div><span>Pending Leaves</span><h3>{leaves.filter(l => l.status === "Pending").length}</h3></div>
                    <div className="stat-box orange"><div className="stat-icon" aria-hidden="true"></div><span>Avg. Performance</span><h3>4.8/5</h3></div>
                  </div>
                </div>
              )}

              {activeTab === "workforce" && <WorkforcePlanning />}
              {activeTab === "candidateEvaluation" && <CandidateEvaluation />}
              {activeTab === "screening" && <ScreeningInterview />}
              {activeTab === "onboardingSetup" && <OnboardingSetup />}
              {activeTab === "learning" && <LDManagement />}
              {activeTab === "succession" && <SuccessionPlanning />}
              {activeTab === "grievanceResolution" && <GrievanceResolution />}
              {activeTab === "exitProcessing" && <ExitProcessing />}
                        ...
                        {performances.length > 0 ? (
                          performances.map((p) => (
                            <tr key={p.id}>
                              ...
                              <td><span style={{ color: '#00d2ff' }}>★ {p.rating}/5</span></td>
                              <td>{p.comments}</td>
                              <td>{p.review_date ? new Date(p.review_date).toLocaleDateString() : 'N/A'}</td>
                            </tr>
                          ))
                        ) : (
                          <tr>
